...
- **数据**:启动时在进程内复用与 `report` 完全相同的聚合管道(全部 adapters + 相同 dedup 口径)与持久 Device ID,构建当日快照后直接走 store 层 latest-wins 落库——不发自环 HTTP、不需要 User Token。首次运行检测到无历史数据时自动回溯(默认近 30 天,`--since` 可改,受 550 天回溯上限约束);显式传 `--since` 时无条件重放该区间(按天 latest-wins、空日跳过),用于补回漏掉的某天,显式空串等价于未传。服务常驻期间按 `--refresh` 间隔(默认 15 分钟)重新聚合并覆盖当日。
