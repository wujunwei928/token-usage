...
// Profile is Copilot's report profile for the shared pipeline: Sunday weeks,
// key-swap sessions, filter before summarize — the zero value.
var Profile = common.ReportProfile{}

// adapter exposes the Copilot loader through the unified Agent Adapter
// interface (ADR 0009), with the raw --offline pricing semantics its spec
// carried.
type adapter struct {
	pricing *core.PricingMap
}

func (adapter) Agent() string { return "copilot" }
...
func init() {
	common.RegisterAgent("copilot", func(shared *core.SharedArgs) common.Adapter {
