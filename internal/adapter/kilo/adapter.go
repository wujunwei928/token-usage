...
// Profile is Kilo's report profile for the shared pipeline: Sunday weeks,
// key-swap sessions, filter before summarize — the zero value.
var Profile = common.ReportProfile{}

// adapter exposes the Kilo loader through the unified Agent Adapter
// interface (ADR 0009). Pricing keeps Kilo's display-mode gate.
type adapter struct {
	pricing *core.PricingMap
}

func (adapter) Agent() string { return "kilo" }
...
func init() {
	common.RegisterAgent("kilo", func(shared *core.SharedArgs) common.Adapter {
