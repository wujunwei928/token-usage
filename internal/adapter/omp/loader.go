...
// LoadOptions carries what the omp loader needs from the CLI layer.
type LoadOptions struct {
	Shared *core.SharedArgs
	// CustomPath is the --omp-path value (comma-separated session roots).
	CustomPath *string
	// Pricing supplies token pricing; nil skips pricing entirely (display
	// mode), matching the pi loader's unused-pricing behavior.
	Pricing *core.PricingMap
}

// LoadEntries discovers, parses, dedupes, and time-orders omp session entries.
func LoadEntries(opts LoadOptions) ([]core.LoadedEntry, error) {
	return core.TrackUsageLoad("oh-my-pi", opts.Shared, func() ([]core.LoadedEntry, error) {
...
func loadEntries(opts LoadOptions) ([]core.LoadedEntry, error) {
	...
	for _, path := range paths {
		...
		loaded := common.ReadFilesParallel(files, shared.SingleThread, func(file string) []core.LoadedEntry {
			...
			if err != nil {
				core.DebugLog(shared, "Failed to read omp session file "+file+": "+err.Error())
				return nil
			}
			return fileEntries
		})
