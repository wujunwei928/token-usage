...
// Profile is Kimi's report profile for the shared pipeline: Sunday weeks,
// key-swap sessions, filter before summarize — the zero value.
var Profile = common.ReportProfile{}

// adapter exposes the Kimi loader through the unified Agent Adapter
// interface (ADR 0009), with the unified pricing semantics.
type adapter struct {
	pricing *core.PricingMap
}

func (adapter) Agent() string { return "kimi" }
...
func init() {
	common.RegisterAgent("kimi", func(shared *core.SharedArgs) common.Adapter {
