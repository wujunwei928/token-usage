...
**Agent Adapter**:
一个 agent(如 claude、codex、opencode、zcode、omp、cline)的数据源适配器,负责发现该 agent 的本地用量记录(JSONL 会话文件或分析数据库)、解析其私有格式、产出统一的 Usage Entry。
...
**Detected**:
All-Report 标题行 `Detected:` 列出的"本机存在数据"的 agent 集合;多数 agent 以加载出非空 Usage Entry 为准,qwen/zcode/opencode/omp/cline 以数据源存在(HasData)即算。
