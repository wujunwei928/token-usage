...
单二进制 + SQLite 文件即可运行,前端资源(ECharts/样式/模板)全部内嵌,无外部文件依赖。
...
| 命令 | 说明 |
|------|------|
| `serve` | 启动服务;`-addr` 监听地址,`-db` SQLite 路径,`-pricing` 价格表覆盖文件 |
| `add-user` | 建用户并签发一次性 report token;`--name` 必填,`--city` `--password` 可选 |
| `dump-pricing` | 导出当前生效价格表为 model-prices.json(可编辑后用 `-pricing` 回喂) |
...
调价流程:`dump-pricing` 导出 → 编辑 → 用 `-pricing` 指向后重启。
...
- 服务端代码:`cmd/server`(入口)、`internal/server`(store/api/pricing/query/web)
- 端到端测试:`internal/e2e`(真二进制全链路)
